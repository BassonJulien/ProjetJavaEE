...
public interface CourseRepository extends JpaRepository<Course, Long> {
	...
	@Query(value = "SELECT * FROM intranetPortal.course where intranetPortal.course.class_id=:id ;", nativeQuery = true)
	public List<Course> getCourseListByClass(@Param("id") Long id);
	
	@Modifying
	@Query(value = "SELECT course.name as cname, group_class.name as classname, teacher.name as tname "
			+ "FROM course, group_class, teacher " + "WHERE (teacher.id_teacher = course.teacher_id) "
			+ "AND (course.class_id = group_class.class_id) " + "AND teacher.id_teacher=:idid ;", nativeQuery = true)
	public List<Object[]> getCourseClassList(@Param("idid") Long id);
